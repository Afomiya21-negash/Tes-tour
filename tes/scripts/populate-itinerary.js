...
async function populateItinerary() {
  ...
  try {
    ...
    const [tables] = await connection.execute("SHOW TABLES LIKE 'itinerary'");
    if (tables.length === 0) {
    ...
    // Clear existing data
    await connection.execute('DELETE FROM itinerary');
    ...
    console.log('Itinerary data populated successfully!');
    
    // Verify the data
    const [rows] = await connection.execute('SELECT COUNT(*) as count FROM itinerary');
    console.log(`Total itinerary entries: ${rows[0].count}`);
